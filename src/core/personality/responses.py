...
class ResponseManager:
    ...
    def generate_response(
        ...
        try:
            ...
            formatted_context = self._format_context(context, template.formatting)
            ...
            if emotional_state:
                response = self._add_emotional_context(response, emotional_state)

            # Record response
            self._record_response(response_type, response, context)
    ...
    def _format_context(
        self, context: Dict, formatting: Optional[Dict[str, str]] = None
    ) -> Dict:
        ...
        for key, format_type in formatting.items():
            if key in formatted:
                if format_type == "json":
                    formatted[key] = json.dumps(formatted[key], indent=2)
                elif format_type == "number":
                    formatted[key] = f"{formatted[key]:,.2f}"

        return formatted

    def _add_emotional_context(self, response: str, emotional_state: Dict) -> str:
    ...
    def _record_response(
        self, response_type: ResponseType, response: str, context: Dict
    ):
    ...
    def get_response_history(
        self, response_type: Optional[ResponseType] = None, limit: Optional[int] = None
    ) -> List[Dict]:
        """Get response history with optional filtering"""
        history = self.response_history

        if response_type:
            history = [r for r in history if r["type"] == response_type.value]
    ...
    def get_formatted_response(
        self, response_type: ResponseType, context: Dict, format_type: str = "text"
    ) -> Union[str, Dict]:
        """Get formatted response"""
        response = self.generate_response(response_type, context)
