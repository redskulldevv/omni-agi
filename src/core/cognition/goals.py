...
class GoalManager:
    ...
    async def get_goal_progress_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'progress_by_type': {
                goal_type.value: {
                    'count': len([g for g in self.goals.values() if g.type == goal_type]),
                    'avg_progress': sum(g.progress for g in self.goals.values() if g.type == goal_type) / 
                                 len([g for g in self.goals.values() if g.type == goal_type] or [1])
                }
            ...
            'high_priority_goals': [
                ...
                for g in sorted(self.goals.values(), key=lambda x: x.priority, reverse=True)[:5]
            ]
        }