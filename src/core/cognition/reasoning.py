...
class MarketStrategy(Strategy):
    ...
    async def analyze(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'action': self._determine_action(price_trend, volume_analysis, sentiment),
...
class ReasoningEngine:
    ...
    async def make_decision(
        ...
        try:
            ...
            if not strategy:
                raise ValueError(f"No strategy found for decision type: {decision_type}")
            ...
            for key, limit in constraints['value_limits'].items():
                if key in constrained_context:
                    constrained_context[key] = max(
                        min(constrained_context[key], limit['max']),
                        limit['min']
                    )
