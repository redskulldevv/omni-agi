...
class SocialAnalytics:
    ...
    async def detect_trends(
        ...
        try:
            ...
            # Filter trends
            trending_mentions = [
                mention for mention, count in mentions.items()
                if count >= min_mentions
            ]
            
            trending_hashtags = [
                tag for tag, count in hashtags.items()
                if count >= min_mentions
            ]
        ...
        try:
            ...
            return {
                'summary': {
                    ...
                    'sentiment_trend': self._get_sentiment_label(avg_metrics.sentiment_score),
    ...
    def _calculate_metrics_trend(
        self,
        metrics_history: List[EngagementMetrics]
    ) -> Dict[str, str]:
    ...
    def _generate_recommendations(
        ...
        if metrics.sentiment_score < 0:
            recommendations.append(
                "Focus on positive messaging to improve sentiment"
            )
