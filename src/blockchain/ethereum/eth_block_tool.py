...
def fetch_and_analyze_ethereum_transaction(
...
) -> Union[Tuple[str, Dict[str, Any]], str, Dict[str, Any]]:
    """
    ...
    Args:
        ...
        return_type (str, optional): Specify the return type. Defaults to "both".
                                     Options: "both", "string", "dict".

    Returns:
        Tuple[str, Dict[str, Any]] or str or Dict[str, Any]: Depending on the return_type,
                                    returns a tuple containing a status message (str) and
                                    a dictionary with the fetched and analyzed transaction
                                    data (Dict[str, Any]), or only the status message (str),
                                    or only the transaction data (Dict[str, Any]).

    Raises:
        ValueError: If the transaction data cannot be fetched or analyzed.
    """
    api_key = os.getenv("ETHERSCAN_KEY")
    logger.info(f"Fetching Ethereum transaction data for hash: {tx_hash}")
    ...
    try:
        ...
        if transaction_data.get("result") is None:
            logger.error(f"Transaction data not found for hash: {tx_hash}")
            if return_type == "both":
        ...
        logger.info(f"Successfully fetched transaction data for hash: {tx_hash}")
        ...
        if return_type == "both":
            ...
            return (
                ...
                f"Transaction data fetched and analyzed successfully for the tx hash: "
                f"{tx_hash} || Data Available:\n"
                f"{format_dict_row_by_row(transaction_data)}"
            )
        elif return_type == "dict":
        ...
        logger.error(f"Value error while processing transaction data: {val_err}")
        if return_type == "both":
            return (
...
# out = fetch_and_analyze_ethereum_transaction(
#     "0xa55de4c4362ac656ee0d82a16c4332d5e93e2818b896d411e2b9f79c61f8a217",
#     return_type="string"
# )
# print(out)
